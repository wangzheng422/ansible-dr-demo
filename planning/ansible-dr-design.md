...
* **实现方式**: 在主 OpenShift 集群上，通过一个定制的 **Python 事件转发器 (k8s_event_forwarder.py)** 来实现。
  ...
  * 容器镜像基于 Red Hat UBI 8 (`registry.access.redhat.com/ubi8/python-39`) 构建，确保了环境的稳定性和安全性。为了增强安全性并解决在某些环境中（如 rootless Podman）可能出现的权限问题，Dockerfile 已被修改为创建一个名为 `appuser` 的非 root 用户，并使用此用户来安装依赖和运行应用。
